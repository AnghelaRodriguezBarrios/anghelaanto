...
<body
		...
		<div class="row flex-nowrap">
			...
			<div class="col p-0">
				...
				<div class="container">
					...
					<!-- Coloca el botón en el lugar deseado -->
					<button id="descargarPDF" class="btn btn-primary">Descargar
						PDF</button>
					<button id="descargarExcel" class="btn btn-success">Descargar
						Excel</button>
				</div>
  ...
  botonDescargarPDF.addEventListener('click', () => {
    ...
    const options = {
      filename: 'Listado_tabla_Person.pdf',
  ...
  // Función para descargar la tabla en formato Excel (CSV)
  function descargarExcel() {
    ...
    const header = Array.from(tablaEstudiantes.querySelectorAll('thead th')).map((th) => th.textContent);
    datos.unshift(header);
    ...
    link.setAttribute('download', 'tabla_estudiantes.csv');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  }

  // Obtén una referencia al botón Descargar Excel
  const botonDescargarExcel = document.getElementById('descargarExcel');

  // Agrega el evento click al botón Descargar Excel
  botonDescargarExcel.addEventListener('click', descargarExcel);
</script>
</body>
</html>